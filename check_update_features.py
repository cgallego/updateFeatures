...
from mylocalbase import myengine
import datetime  
...
class CheckUpdate(object):
    ...
    def check_update_selected(self, file_masterlist, chgSegfile, chgDyn, chgMor, chgTex, chgT2, chgStage1):
        ...
        while ( line ) : 
            ...
            if chgSegfile:
                ...
                segment_records = mydatabase.Segment_record(lesion_id, Send2DB.loadDisplay.lesion_bounds[0], Send2DB.loadDisplay.lesion_bounds[1], Send2DB.loadDisplay.lesion_bounds[2], Send2DB.loadDisplay.lesion_bounds[3], Send2DB.loadDisplay.lesion_bounds[4], Send2DB.loadDisplay.lesion_bounds[5],
            ...
            if chgDyn:
                ...
                #############################
                [dyn_inside, dyn_contour] = Send2DB.extract_dyn(series_path, phases_series, lesion3D)
                 
                self.session = self.Session() #instantiate a Session
                # Send to database lesion info
                dyn_records = mydatabase.Dynamic_features(lesion_id, dyn_inside['A.inside'],dyn_inside['alpha.inside'], dyn_inside['beta.inside'], dyn_inside['iAUC1.inside'], dyn_inside['Slope_ini.inside'], dyn_inside['Tpeak.inside'],
            ...
            if chgMor:
                ...
                #############################
                morphofeatures = Send2DB.extract_morph(series_path, phases_series, lesion3D)
                # Morphology
                self.session = self.Session() #instantiate a Session
                # Send to database lesion info
                morp_records = mydatabase.Morpho_features(lesion_id, morphofeatures['min_F_r_i'], morphofeatures['max_F_r_i'], morphofeatures['mean_F_r_i'], morphofeatures['var_F_r_i'], morphofeatures['skew_F_r_i'], morphofeatures['kurt_F_r_i'],
            ...
            if chgTex:            
                ...
                #############################
                texturefeatures = Send2DB.extract_text(series_path, phases_series, lesion3D) 
                
                self.session = self.Session() #instantiate a Session
                # Send to database lesion info
                tex_records = mydatabase.Texture_features(lesion_id, texturefeatures['texture_contrast_zero'], texturefeatures['texture_contrast_quarterRad'], texturefeatures['texture_contrast_halfRad'], texturefeatures['texture_contrast_threeQuaRad'], 
            ...
            if chgStage1:
                ...
                self.session = self.Session() #instantiate a Session
                # Send to database lesion info
                stage1_records = mydatabase.Stage1_record(lesion_id, d_euclideanNodes, earlySE, dce2SE, dce3SE, lateSE, ave_T2, network_measures)
            ...
            if chgT2: 
                ...
                if T2SeriesID != 'NONE':
                    ...
                    #[T2_muscleSI, muscle_scalar_range, bounds_muscleSI, T2_lesionSI, lesion_scalar_range, LMSIR, morphoT2features, textureT2features] = Send2DB.T2_extract(T2SeriesID, path_T2Series, lesion3D, pathSegment, nameSegment, sideBreast)
                    [T2_muscleSI, muscle_scalar_range, bounds_muscleSI, T2_lesionSI, lesion_scalar_range, LMSIR, morphoT2features, textureT2features] = Send2DB.T2_loadupdate(T2SeriesID, path_T2Series, lesion3D, pathSegment, nameSegment, sideBreast, bounds_muscleSI)
                    ...
                    self.session = self.Session() #instantiate a Session
                    # Send to database lesion info
                    t2_records = mydatabase.T2_features(lesion_id, str(RetrieveDB.T2info.find_t2_signal_int), str(list(Send2DB.load.T2dims)), str(list(Send2DB.load.T2spacing)), str(Send2DB.load.T2fatsat), mean(T2_muscleSI), std(T2_muscleSI), str(muscle_scalar_range), str(bounds_muscleSI), mean(T2_lesionSI), std(T2_lesionSI), str(lesion_scalar_range), LMSIR, 
